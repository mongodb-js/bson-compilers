...
class Visitor extends ECMAScriptVisitor {
    ...
    if (ctx.propertyNameAndValueList()) {
      ...
      if (ctx.type.argsTemplate) {
        args = ctx.type.argsTemplate(properties.map((pair) => [
          this.visit(pair.propertyName()), this.visit(pair.singleExpression())
        ]));
      }
    }
  ...
  visitArrayLiteral(ctx) {
    ...
    if (ctx.elementList()) {
      ...
      if (ctx.type.argsTemplate) {
        args = ctx.type.argsTemplate(children.map((c) => (this.visit(c))));
      } else {
        args = children.map((c) => (this.visit(c))).join(', ');
      }
    }
  ...
  getTyped(actual) {
    ...
    if (typed.type === undefined) {
      while (typed.singleExpression()) {
        typed = typed.singleExpression();
  ...
  castType(expected, actual) {
    ...
    const typed = this.getTyped(actual);

    // If the types are exactly the same
    if (
      expected.indexOf(typed.type) !== -1
      || expected.indexOf(typed.type.id) !== -1
    ) {
      return result;
    }
    ...
    if (
      expected.indexOf(this.Types._numeric) !== -1
      && (
        numericTypes.indexOf(typed.type) !== -1
        || (
          typed.type.id === 'Long'
          || typed.type.id === 'Int32'
          || typed.type.id === 'Double'
        )
      )
    ) {
      return result;
  ...
  processBinary(ctx) {
    ...
    const rhs = symbolType.argsTemplate
      ? symbolType.argsTemplate(lhs, bytes, typeStr)
      : `(${singleQuoteStringify(bytes)}${typeStr === null ? '' : `, ${typeStr}`})`;
